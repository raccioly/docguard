...
function checkUntrackedTodos(projectDir, config) {
  ...
  for (const todo of todos) {
    // ⚡ Bolt: Hoist expensive TODO text manipulation outside the document tracking loop
    ...
    const isTracked = trackingContent.some(doc => {
      ...
      // ⚡ Bolt: Use precomputed doc.contentLower instead of calling .toLowerCase() on N*M iterations
  ...
  for (const file of trackingFiles) {
    ...
    if (existsSync(fullPath)) {
      try {
        ...
        // ⚡ Bolt: Precompute content.toLowerCase() to prevent N^2 bottleneck in checkUntrackedTodos loop
        const contentLower = content.toLowerCase();
        docs.push({ file, content, contentLower });
      } catch { /* ignore */ }
    }
  }
