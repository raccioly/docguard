...
function getCodeCommitsSince(date, dir) {
  try {
    ...
    const result = execFileSync('git', ['log', '--since=' + isoDate, '--oneline', '--diff-filter=M', '--', '*.js', '*.mjs', '*.ts', '*.tsx', '*.py', '*.java', '*.go'], { cwd: dir, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
...
function isGitRepo(dir) {
  try {
    execFileSync('git', ['rev-parse', '--is-inside-work-tree'], { cwd: dir, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
    return true;
  } catch {
    return false;
  }
...
function getTotalCommits(dir) {
  try {
    return parseInt(execFileSync('git', ['rev-list', '--count', 'HEAD'], { cwd: dir, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim()) || 0;
  } catch {
    return 0;
  }
}
...
function getRecentCodeCommits(dir, count = 5) {
  try {
    const result = execFileSync('git', ['log', '-' + count, '--format=%h %aI %s', '--', '*.js', '*.mjs', '*.ts', '*.tsx', '*.py', '*.java'], { cwd: dir, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
    return result ? result.split('\n') : [];
  } catch {
    return [];
  }
...
export function validateFreshness(dir, config) {
  ...
  if (existsSync(driftPath)) {
    ...
    try {
      const recentDrifts = execFileSync('git', ['log', '-5', '--all', '-p', '--', '*.js', '*.mjs', '*.ts', '*.tsx', '*.py'], { cwd: dir, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
      const driftCount = (recentDrifts.match(/DRIFT:/g) || []).length;
