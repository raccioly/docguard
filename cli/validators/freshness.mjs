...
function getCodeCommitsSince(date, dir) {
  try {
    ...
    const result = logOutput.trim().split('\n').filter(line => line.length > 0).length.toString();
  ...
  if (existsSync(driftPath)) {
    ...
    try {
      ...
      try {
        ...
        if (err.stdout) logOutput = err.stdout.toString();
      }
      const matches = logOutput.match(/(?:DRIFT:)/g);
      const driftCount = matches ? matches.length : 0;
