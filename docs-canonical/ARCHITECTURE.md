...
Completion also supports reviewed maintenance of a verified living spec. It reconciles from the prior reviewed revision and appends a `verified → verified` outcome only when a linked source, test, canonical document, or decision changed. The generated registry, active-context projection, and implementation-outcome section cannot create a self-sustaining completion loop.
