...
4. Badge test isolation is corrected: the two legacy command cases use disposable projects, the current Node executable and a closed tooling PATH, with cleanup on completion. Other tests still require a broader isolation audit.
...
The read-only dispatch follow-up at f1f8b4f passed [all four supported-runtime CI jobs and security checks](https://github.com/raccioly/docguard/actions/runs/34855267949). A subsequent test-only isolation correction moves both legacy badge cases out of the source checkout. Both badge cases pass on Node 18 and 24, and neither run changes generated source-tree skills; unrelated tests are filtered in these focused runs. Final CI must be checked at the resulting commit before merging.


### Scoped requirement correction

Document-qualified identities now prevent coverage sharing between features. Nine paired regression cases cover ambiguous duplicates, one or both qualified targets, annotations and labels, wrong qualifiers, unique bare compatibility and repeated definitions within one document. Six cases fail against the immutable pre-fix CLI; all nine pass after the correction, along with 70 existing trace tests. Lifecycle and arbitrary verification links remain out of scope.
