...
describe('benchmark manifest contract', () => {
  ...
  it('rejects unknown fields, duplicate IDs, and cross-split leakage', () => {
    ...
    const leakage = fresh();
    leakage.cases[2].repositoryGroup = leakage.cases[0].repositoryGroup;
  ...
  it('rejects missing controls, floating Git revisions, and unsafe paths', () => {
    ...
    floating.cases[2].source = { kind: 'git', url: 'https://github.com/example/project.git', revision: 'main', license: 'MIT' };
  ...
  it('rejects private Git endpoints and hostile benchmark configuration', () => {
    for (const url of [
      ...
      const manifest = fresh();
      manifest.cases[2].source = { kind: 'git', url, revision: 'a'.repeat(40), license: 'MIT' };
