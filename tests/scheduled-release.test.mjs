...
describe('scheduled release workflow', () => {
  ...
  it('prevalidates release identity, synchronized versions, and changed paths before push', () => {
    ...
    for (const path of [
      ...
      'docs/llms.txt',
      'llms.txt.bak',
      // The release job does NOT regenerate the llms bundles — the per-pull-request
      // drift test keeps them current, and carrying them here conflicts with any
      // pull request merged while the release PR waits for approval. Re-admitting
      // them is how that outage comes back.
      'llms.txt',
      'llms-full.txt',
    ]) assert.equal(RELEASE_PATH_ALLOWLIST.test(path), false, `release allowlist admitted ${path}`);
    assert.equal(/^\s*npm run llms\b/m.test(workflow), false,
      'the release job must not regenerate the llms bundles — see the comment in scheduled-release.yml');
    assert.equal(/^\s*git add[^\n]*llms/m.test(workflow), false,
      'the release commit must not stage the llms bundles');
