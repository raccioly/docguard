...
describe('llms --full and memory --pack (v0.29)', () => {
  ...
  beforeEach(() => {
    ...
    // `version` here is the .docguard.json SCHEMA version. It is set to a value
    // that is deliberately not the tool version, so the footer test below can
    // tell which one the generator quoted.
    writeFileSync(join(tmpDir, '.docguard.json'),
      JSON.stringify({ profile: 'starter', projectName: 'llms-fixture', version: '0.6' }));
  ...
  it('both forms stamp the TOOL version, not the config schema version', () => {
    // Regression: the footer read `config.version` — the .docguard.json schema
    // version — so every generated llms.txt announced "Generated by DocGuard
    // v0.6" no matter which release produced it.
    const toolVersion = JSON.parse(
      readFileSync(new URL('../package.json', import.meta.url), 'utf-8')).version;
    const configVersion = JSON.parse(
      readFileSync(join(tmpDir, '.docguard.json'), 'utf-8')).version;
    assert.notEqual(toolVersion, configVersion, 'fixture must keep the two versions distinct');

    for (const args of [['llms'], ['llms', '--full']]) {
      const res = run(args, tmpDir);
      assert.equal(res.status, 0, res.stderr);
      const out = readFileSync(join(tmpDir, args.includes('--full') ? 'llms-full.txt' : 'llms.txt'), 'utf-8');
      assert.ok(out.includes(`Generated by DocGuard v${toolVersion} |`),
        `${args.join(' ')} must stamp the tool version`);
      assert.ok(!out.includes(`Generated by DocGuard v${configVersion} |`),
        `${args.join(' ')} must not stamp the config schema version`);
    }
  });
