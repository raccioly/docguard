...
## 2024-05-24 - Pre-lowercase large text structures to prevent repeated lowercasing
**Learning:** Calling `.toLowerCase()` repeatedly on the same massive text payload (e.g. `allDocContent`) inside multiple validation checks and loops incurs redundant and measurable overhead.
**Action:** When working with massive text bodies, pre-lowercase them at the highest possible scope (`allDocContent.toLowerCase()`) and pass the resulting string as a local variable down to helper functions/loops to avoid redundant memory allocations and iteration overhead.
