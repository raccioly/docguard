...
## 2024-05-25 - O(N*M) Array.find bottlenecks
**Learning:** Using `Array.find` inside nested loops to match entity properties (e.g., schemas and fields) causes an $O(N^2)$ algorithmic bottleneck, especially noticeable when parsing large documents like OpenAPI specifications.
**Action:** Always precompute a `Map` of the target array for $O(1)$ lookups based on the identifier key before entering the nested loops.
