...
## 2024-06-26 - Single-pass Set comparison for matching lists
**Learning:** Using nested `.filter` and `.some` to compare two lists (like doc tests vs code tests) is O(N*M) and does redundant work (like repeatedly extracting basenames).
**Action:** Pre-compute expensive operations (like `basename()`) mapping lists to objects, and use a single-pass loop with `Set`s to track matches, improving scale and performance.
