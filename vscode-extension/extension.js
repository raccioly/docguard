...
function findSpecguard(workspaceDir) {
  ...
  const localBinCmd = path.join(workspaceDir, 'node_modules', '.bin', 'specguard.cmd');
...
function execSpecguard(workspaceDir, argsArray) {
  ...
  try {
    if (localBin) {
      ...
      // Try npx, with fallback to npx.cmd on Windows
      try {
        return execFileSync('npx', ['-y', 'specguard', ...argsArray], options);
      } catch (err) {
        if (err.code === 'ENOENT' && process.platform === 'win32') {
          return execFileSync('npx.cmd', ['-y', 'specguard', ...argsArray], options);
        }
        throw err;
      }
    }
