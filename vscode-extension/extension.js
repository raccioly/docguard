...
function execSpecguard(workspaceDir, argsArray) {
  ...
  try {
    if (process.platform === 'win32') {
      try {
        return execFileSync(command + '.cmd', finalArgs, execOptions);
      } catch (e) {
        if (e.code === 'ENOENT') {
          return execFileSync(command, finalArgs, execOptions);
        }
        throw e;
      }
    }
...
function runCommand(cmdString) {
  ...
  outputChannel.appendLine(`$ specguard ${cmdString}\n`);

  // Basic split for commands run via UI string input
  const cmdArgs = (cmdString.match(/(?:[^\s"]+|"[^"]*")+/g) || []).map(a => a.replace(/^"|"$/g, ''));
