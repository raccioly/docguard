...
function execSpecguard(workspaceDir, argsArray) {
  ...
  if (localBin) {
    ...
    execArgs = [localBin, ...argsArray];
  } else {
    const isWindows = process.platform === 'win32';
    execPath = isWindows ? 'npx.cmd' : 'npx';
    execArgs = ['-y', 'specguard', ...argsArray];
  }
  ...
  try {
    return execFileSync(execPath, execArgs, {
      cwd: workspaceDir,
      encoding: 'utf-8',
      env: { ...process.env, NO_COLOR: '1', ELECTRON_RUN_AS_NODE: '1' },
      timeout: 30000,
    });
  } catch (e) {
    // If npx on Windows fails because npx.cmd wasn't found but npx might exist, fallback isn't needed since we use npx.cmd directly.
