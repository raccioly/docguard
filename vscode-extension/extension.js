...
function findSpecguard(workspaceDir) {
  ...
  // Fallback to extension-less check on Windows just in case
  const extensionLess = path.join(workspaceDir, 'node_modules', '.bin', 'specguard');
  if (fs.existsSync(extensionLess)) {
    return process.platform === 'win32' ? extensionLess + '.cmd' : extensionLess;
  }
